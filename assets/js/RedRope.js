...
//The Black Circle
var circle = new Path.Circle(view.center, 100);
	circle.strokeWidth = 10;
	circle.strokeColor = 'black';

//The Black Ball (at the end of The Red Rope)	
var ball = new Path.Circle(view.center, 10);
	ball.strokeWidth = 20;
...
function onMouseMove(event) {
	...
	if((mousePos-circle.position).length <= 110){
		mousePos = setDistance(mousePos, circle.position, 110);
    }
}
...
function onFrame(event) {
	for (var i = 0; i < points - 1; i++) {
	    ...
	    //Add gravity
		rope.segments[i+1].point += new Point(0, 1);
	}
