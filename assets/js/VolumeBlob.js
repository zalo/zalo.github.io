...
//The Black Circle
var circle = new Path.Circle(view.center, 305);
...
function onFrame(event) {
	...
	for (var j = 0; j < 10; j++) {
	    ...
	    //Handle collisions
    	for (var i = 0; i < points; i++) {
    		if((blob.segments[i].point - mousePos).length < 80){
    		    blob.segments[i].point = setDistance(blob.segments[i].point, mousePos, 80);
    		}
    		if((blob.segments[i].point - view.center).length > 300){
    		    blob.segments[i].point = setDistance(blob.segments[i].point, view.center, 300);
    		}
    	}
	}
