var Ellipsoid = function (environment, focus1Pos, focus2Pos, minorAxisIn, invertEllipsoid = false) {
  this.environment       = environment;
  this.sphereGeometry    = new THREE.SphereBufferGeometry(0.5, 50, 50);
  this.blue              = new THREE.MeshPhongMaterial  ({ color: 0x3399dd });
  this.transparentGreen  = new THREE.MeshLambertMaterial({ color: 0x777777, transparent: true, opacity: 0.5, side: invertEllipsoid?THREE.BackSide:THREE.FrontSide });
  ...
  this.toSphereSpace = function (inVector) {
    inVector.applyMatrix4(this.sceneToSphereSpace);
  }

  this.toSceneSpace = function (inVector) {
    inVector.applyMatrix4(this.sphereToSceneSpace);
  }
  ...
  this.raytraceEllipsoid = function(rayOrigin, rayDirection, hitPoint, hitNormal){
    ...
    let intersectionTime = this.intersectLineSphere(sphereSpaceRayOrigin, sphereSpaceRayDirection, new THREE.Vector3(), 0.25, false);
    if(intersectionTime > 0){
      ...
      console.log("Negative Intersection time!");
    }
  }
